...
(function(){
  ...
  css.textContent=`
    ...
    .bh-conflict{display:block;margin-top:6px;padding:5px 8px;border-radius:9px;background:#fff0e8;color:#a6532b;font-size:10px;font-weight:900}
  ...
  async function setStatus(id,newStatus){
    if(!window.sb)return;
    const r=await window.sb.from("agenda_items").update({status:newStatus}).eq("id",id);
  ...
  function decorate(){
    ...
    grid.querySelectorAll(".bh-agenda").forEach(el=>{
      const id=el.dataset.agendaId;if(!id||el.dataset.phase2Ready)return;
    ...
    grid.querySelectorAll(".day").forEach(day=>{
      ...
      day.addEventListener("drop",async e=>{
        e.preventDefault();day.classList.remove("bh-drop-target");
        const id=e.dataTransfer.getData("text/plain")||dragId;if(!id)return;
        const target=day.querySelector(".day-head")?.dataset?.date;if(!target)return;
        const item=await getItem(id);if(item&&item.date!==target)await moveToDate(id,target);
      });
    });
    ...
    if(m){
      const base=new Date(`${m[3]}-${m[2]}-${m[1]}T12:00:00`);
      days.forEach((d,i)=>{const x=new Date(base);x.setDate(x.getDate()+i);d.querySelector(".day-head")?.setAttribute("data-date",x.toISOString().slice(0,10))});
    }

    // Conflict badges are read from the real Supabase row, so they stay correct across devices.
    grid.querySelectorAll(".bh-agenda").forEach(async el=>{
      const id=el.dataset.agendaId;if(!id||el.querySelector(".bh-conflict"))return;
      const item=await getItem(id);
      if(item?.conflict){
        el.classList.add("bh-overload");
        const badge=document.createElement("div");badge.className="bh-conflict";badge.textContent="⚠ "+(item.conflict_reason||"Conflito de agenda");el.appendChild(badge);
      }
    });
  }
